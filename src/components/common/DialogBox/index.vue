  ...
  <div class="inline-block">
    ...
    <el-dialog
          ...
          <el-row :gutter="20">
            ...
            >
              ...
              <el-col v-if="option.type==='upload'" :span="option.span || 20" class="search-bar-col" >
                <el-form-item
                  :label="option.label">
                  <uploadCommon ref="fileUp" :file-type-list="option.FileTypeList" :file-name="option.name" :file-type="option.uploadType" @remove="removeFile($event, option.name)" @success="appendFile($event, option.name)"/>
                </el-form-item>
              </el-col>
            </div>
          </el-row>
      ...
      <span slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" @click="reset">{{ cancelText }}</el-button>
      </span>
    </el-dialog>
  </div>
</template>
...
export default {
  ...
  methods: { // 方法
    ...
    valueChange(valueList, option) {
      console.log(valueList, option)
      // 记录上一次的数组,
      if (!option.multiple) {
        if (this.testSame) {
          const str = option.name === 'sourceCode' ? 'destCode' : 'sourceCode'
          if (this.params[str] === valueList) {
    ...
    appendFile(formData, name) {
      this.formData.set(name, formData.get(name))
    },
    removeFile(formData, name) {
